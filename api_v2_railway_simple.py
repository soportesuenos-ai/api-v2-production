...
async def tiktok_trending(country: str = "US", keyword: str = ""):
    ...
    try:
        ...
        run_input = {
            "hashtags": [keyword if keyword else "viral"],
            "resultsPerPage": 20,
        ...
        for item in items:
            anuncios.append({
                "titulo": item.get("text", "")[:100],
                "autor": item.get("authorMeta", {}).get("name", ""),
        ...
        anuncios = sorted(anuncios, key=lambda x: x["vistas"], reverse=True)[:15]
