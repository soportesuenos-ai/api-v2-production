"""
API v2: Market Discovery + SerpAPI + Google Trends bilingue + Meta Ads exacto
"""
...
def obtener_trends_para_query(query: str) -> dict:
    try:
...
def obtener_google_trends_real(query: str) -> dict:
    ...
    resultado_es = obtener_trends_para_query(query)
    resultado_en = obtener_trends_para_query(query_en) if use_english else None

    if resultado_es and resultado_en:
        if resultado_en["interes"] > resultado_es["interes"]:
            datos = resultado_en
            idioma_usado = "en"
        else:
            datos = resultado_es
...
async def obtener_trends(product: str):
    ...
    return {
        ...
        "meta_ads_url": 'https://www.facebook.com/ads/library/?active_status=active&ad_type=all&country=CL&q="' + product + '"',
        "timestamp": datetime.now().isoformat()
    }
...
async def buscar_meta_ads(query: str):
    url = 'https://www.facebook.com/ads/library/?active_status=active&ad_type=all&country=CL&q="' + query + '"'
    return {
        "query": query,
        "mensaje": "Abre el link para ver anuncios activos en Chile (busqueda exacta)",
        "url_meta_ads": url,
        "timestamp": datetime.now().isoformat()
    }
