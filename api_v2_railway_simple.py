"""
API v2: Market Discovery + SerpAPI Google Shopping + Meta Ads Library (publica)
"""
...
# ===== META ADS LIBRARY (PUBLICA via SerpAPI) =====

async def buscar_meta_ads_serpapi(query: str) -> dict:
    """Busca anuncios en Facebook/Instagram via SerpAPI Facebook Ads"""
    api_key = os.environ.get("SERPAPI_KEY", "")
    if not api_key:
        return {"error": "SERPAPI_KEY no configurado"}
    ...
    try:
        ...
        anuncios_raw = data.get("ads_library", data.get("results", []))

        anunciantes = {}
        for ad in anuncios_raw:
            page = ad.get("page_name", ad.get("advertiser", "Desconocido"))
            ...
            plataformas = ad.get("publisher_platforms", ad.get("platforms", []))
            for p in plataformas:
                anunciantes[page]["plataformas"].add(p)
            anunciantes[page]["anuncios"].append({
                "texto":      ad.get("ad_body", ad.get("description", "")),
                "titulo":     ad.get("ad_title", ad.get("title", "")),
                "inicio":     ad.get("start_date", ""),
                "plataformas": plataformas,
                "thumbnail":  ad.get("thumbnail", ad.get("image", "")),
                "url":        ad.get("ad_url", ad.get("url", ""))
            })
        ...
        return {
            ...
            "top10": top10,
            "raw_count": len(anuncios_raw)
        }

    except Exception as e:
        return {"error": str(e), "total_anunciantes": 0, "top10": []}
...
async def buscar_meta_ads(query: str):
    ...
    resultado = await buscar_meta_ads_serpapi(query)

    if "error" in resultado and resultado.get("total_anunciantes", 0) == 0:
        raise HTTPException(status_code=500, detail=resultado["error"])

    return {
        "query": query,
        "total_anunciantes": resultado.get("total_anunciantes", 0),
        "top10": resultado.get("top10", []),
        "timestamp": datetime.now().isoformat()
    }
