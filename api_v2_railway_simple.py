...
async def tiktok_trending(country: str = "US", category: str = ""):
    """Obtiene productos trending de TikTok Creative Center via Apify"""
    apify_token = os.environ.get("APIFY_TOKEN", "")
    if not apify_token:
        raise HTTPException(status_code=500, detail="APIFY_TOKEN no configurado")

    try:
        # Usar el actor de TikTok Creative Center Top Products
        actor_id = "apify~tiktok-creative-center-scraper"
        run_input = {
        ...
        async with httpx.AsyncClient() as client:
            ...
            if not run_id:
                raise Exception("No se pudo iniciar el run de Apify")
            ...
            for _ in range(10):
                ...
                if status == "SUCCEEDED":
                    ...
                    raise Exception(f"Run fallido: {status}")
        ...
        for item in items:
            productos.append({
                "nombre": item.get("productName", item.get("name", "")),
                "categoria": item.get("industry", item.get("category", "")),
                "vistas": item.get("videoViews", item.get("views", 0)),
                "crecimiento": item.get("growth", ""),
                "thumbnail": item.get("cover", item.get("thumbnail", "")),
                "url_tiktok": f"https://ads.tiktok.com/business/creativecenter/top-products/pc/en?country={country}"
            })
